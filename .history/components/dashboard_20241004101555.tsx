  ...
  const handleTaskStatusChange = (taskId: string, newStatus: 'completed' | 'pending') => {
    setTasks(prevTasks => {
      ...
      for (const category in updatedTasks) {
        updatedTasks[category as keyof typeof updatedTasks] = updatedTasks[category as keyof typeof updatedTasks].map(task => 
              ...
              {tabs.map((tab) => (
                <TabsContent key={tab} value={tab}>
                  <div className="space-y-4">
                    {(tasks[tab as keyof typeof tasks] ?? []).map((task) => (
                      <div key={task.id} className="flex items-center">
                        <div className="relative">
                          {task.status === 'completed' ? (
                            <button 
                              className="p-2 bg-green-500 text-white rounded-full"
                              onClick={() => handleTaskStatusChange(task.id.toString(), 'incomplete')}
                            >
                            ...
                            <button 
                              ...
                              onClick={() => handleTaskStatusChange(task.id.toString(), 'complete')}
                            >
